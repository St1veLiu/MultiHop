...
class PolicyGradient(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        otherrewards=output['otherrewards']
        ...
        cum_discounted_rewards = 0.7*otherrewards#[0] * self.num_rollout_steps
        cum_discounted_rewards[-1] =cum_discounted_rewards[-1]+ final_reward
        R = 0
    ...
    def rollout(self, e_s, q, e_t, num_steps, visualize_action_probs=False):
        ...
        print(kg.id2relation[q.detach().numpy()[0]])
        ...
        for i in range(e_s.shape[0]):
            ...
            questions[i]=['you tell me who has reationships of '+kg.id2relation[q.detach().cpu().numpy()[i]].split(':')[-1]+' with '+kg.id2entity[e_s.detach().cpu().numpy()[i]].split('_')[-1]] 
        '''
        from transformers import pipeline
        checkpoint = "src/gptmodels/gpt-124M" 

        
        model = pipeline('text-generation', model = checkpoint)
        
        model_=pipeline('feature-extraction', model = checkpoint)
        '''
        
        from zhipuai import ZhipuAI
        client = ZhipuAI(api_key="431cf5c909cc4dc22f08c87505304af7.xXgboTsV584el9PT")
        ...
        for t in range(num_steps):
            ...
            for i in range(e.shape[0]):
                ...
                if len(questions[i][1:])>0:
                    question=question+' if we find'
                    question=question+','.join(questions[i][1:])
                question+'we have found '+kg.id2entity[e.detach().cpu().numpy()[i]].split('_')[-1]
                question=question+' and '
                tmpselects=[]
                uniquees_=[]
                for r in kg.adj_list[e.detach().cpu().numpy()[i]]:
                    for e_ in kg.adj_list[e.detach().cpu().numpy()[i]][r]:
                    
                        v=kg.id2entity[e.detach().cpu().numpy()[i]].split('_')[-1]+' has relation '+kg.id2relation[r].split(':')[-1]+' with '+kg.id2entity[e_].split('_')[-1]
                        tmpselects.append(v)
                        uniquees_.append(e_)
                question=question+','.join(tmpselects)+','
                uniquees_=list(set(uniquees_))
                
                question=question+'you have following choice:'
                validchoices=[]
                for e_ in uniquees_:
                
                    validchoices.append(kg.id2entity[e_].split('_')[-1])
                
                question=question+','.join(validchoices)
                instruction =question

                input_prompt = f"Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n### Instruction:\n{instruction}\n\n### Response:"
                print(len(input_prompt.split(' ')),'**&&&^')
                print(input_prompt)
                response = client.chat.completions.create(model="glm-4", messages=[{"role": "user", "content":instruction}])
                
                as_[i]=[ response.choices[0].message.content]
                
                for _ in as_[i]:
                    print(_)
                
                as_[i]=np.array([np.array(client.embeddings.create(model="embedding-2",input=_).data[0].embedding) for _ in as_[i]])
            ...
            nextr,nexte=action
            preans={}
            for  i in range(nexte.shaope[0]):
                preans[i]='I choose '+kg.id2entity[nexte.detach().cpu().numpy()[i]].split('_')[-1]+','+kg.id2entity[nexte.detach().cpu().numpy()[i]].split('_')[-1]+' has '+  kg.id2relation[nextr.detach().cpu().numpy()[i]].split(':')[-1]+' relation with ' +kg.id2entity[e.detach().cpu().numpy()[i]].split('_')[-1]
                questions[i].append(kg.id2entity[nexte.detach().cpu().numpy()[i]].split('_')[-1]+' (has '+kg.id2relation[nextr.detach().cpu().numpy()[i]].split(':')[-1]+' relation  with '+kg.id2entity[e.detach().cpu().numpy()[i]].split('_')[-1]+')') 
            vectors=[]    
            for i in range(nexte.shaope[0]):
            
                vector = np.array(client.embeddings.create(model="embedding-2",input=preans[i]).data[0].embedding)
                vectors.append(vector)
            vectors=np.array(vectors)
            reward_=[]
            for i in range(e.shape[0]):
            
                vectors[i,:]=vectors[i,:]/np.sum(vectors[i,:]**2)**0.5
                
                as_[i]=as_[i]/np.sum(as_[i]**2,-1,keepdims=True)**0.5
                
                u=vectors[i:i+1,:]@as_[i].transpose(1,0)
                
                reward_.append(np.max(u))
            reward_=torch.tensor(reward_).float()
            
            otherrewards.append(reward_)
